...
void style_ensure(App *app) {
    ...
    g_snprintf(css, sizeof(css),
        ...
        /* left rail: stacked icon+label pill buttons */
        ".nav-btn { background:none; background-image:none; border:none;"
        "  box-shadow:none; outline:none; color:#8b9099; padding:7px 6px;"
        "  margin:2px 0; border-radius:10px; min-height:0; }"
        ".nav-btn label { font-size:9px; }"
        ".nav-btn:hover { background-color:#1d2129; }"
        ".nav-btn:checked { color:#f0f1f3; background-color:#262a32; }"
        ".nav-btn:checked:hover { background-color:#262a32; }"
        ".settings stackswitcher button { background:none; border:none;"
        "  box-shadow:none; outline:none; color:#8b9099; padding:6px 18px;"
        "  min-height:0; font-weight:bold; border-radius:13px; }"
        ".settings stackswitcher button:hover { background-color:#232831; }"
        ".settings stackswitcher button:checked { color:#f0f1f3;"
        "  box-shadow:inset 0 -2px #3da9fc; }"
        ".settings stackswitcher button:checked:hover { background-color:#232831; }"
        ".tp-emptyicon { color:#3a3f48; }"
        /* circular colour swatches in Appearance (drawn with Cairo) */
        ".swatch-btn { padding:3px; margin:0; border:none; box-shadow:none;"
        "  background:none; min-width:0; min-height:0; border-radius:50%%; }"
        ".swatch-btn:hover { background-color:#1d2129; }"
        /* ---- calendar ---- */
